...
public class ReportScripts : MonoBehaviour
{
    ...
    private string scriptPath = @"\VFTest-tests\scripts";
    // aPath = Directory.GetParent(Application.dataPath) + @"\VFTest-tests\Tests";

    // Directory.GetParent gets the one folder above the application.datapath.
    // Since windows uses backslashes, the System.IO will default to take that instead of forward slashes like a supposedly normal guy would do.
    //

    public void Start()
    {
        scriptPath = Directory.GetParent(Application.dataPath) + @"\VFTest-tests\scripts";
    } 
    ...
    {
        string pythonPath = @"C:\Program Files (x86)\python.exe"; // C:\Program Files\Python38\python.exe
